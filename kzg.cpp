...
bool kzg_verifyEval(const G1 &commitment, const CRS &crs, const Fr &i, const Fr &Phi_i, const G1 &witness) {
    ...
    pairing(rhs2, crs.g1 * Phi_i, crs.g2);
...
bool kzg_verifyEval_d2(const G1 &commitment, const CRS &crs, const Fr &i, const Fr &j, const Fr &Phi_ij, const std::pair<G1, G1> &witness) {
    ...
    pairing(rhs3, crs.g1 * Phi_ij, crs.g2);
...
Batching_witness kzg_createWitness_batching(const std::vector<std::vector<Fr>> &funcs, const CRS &crs, const Fr &i) {
    std::vector<Fr> r_vec;
    std::vector<Fr> eval;
    Batching_witness batching;
    G1 witness;
    witness -= witness;
    Fr r;
    for (int t = 0; t < funcs.size(); t++) {
        r.setRand();
        r_vec.push_back(r);
        std::pair<Fr, G1> tmp = kzg_createWitness(funcs[t], crs, i);
        eval.push_back(tmp.first);
        witness += tmp.second * r;
    }
    batching.witness1 = witness;
    batching.r_batching = r_vec;
    batching.evals = eval;
    return batching;
}
...
bool kzg_verifyEval_batching(const std::vector<G1> &commitments, const CRS &crs, const Fr &i, const Batching_witness &batching) {
    GT lhs, rhs;
    lhs = 1;
    pairing(rhs, batching.witness1, crs.PK1.second - crs.g2 * i);
    for (int t = 0; t < commitments.size(); t++) {
        GT tmp;
        pairing(tmp, commitments[t] * batching.r_batching[t], crs.g2);
        lhs *= tmp;
        pairing(tmp, crs.g1 * batching.evals[t] * batching.r_batching[t], crs.g2);
        rhs *= tmp;
    }
    return lhs == rhs;
}
...
Batching_witness_d2 kzg_createWitness_d2_batching(const std::vector<std::vector<std::vector<Fr>>> &funcs, const CRS &crs, const Fr &i, const Fr &j) {
    ...
    for (int t = 0; t < funcs.size(); t++) {
        r.setRand();
        r_vec.push_back(r);
        std::pair<Fr, std::pair<G1, G1>> tmp = kzg_createWitness_d2(funcs[t], crs, i, j);
        eval.push_back(tmp.first);
        witness1 += tmp.second.first * r;
        witness2 += tmp.second.second * r;
    }
    batching.witness1 = witness1;
    batching.witness2 = witness2;
    batching.r_batching = r_vec;
    batching.evals = eval;
    return batching;
}
...
bool kzg_verifyEval_d2_batching(const std::vector<G1> &commitments, const CRS &crs, const Fr &i, const Fr &j, const Batching_witness_d2 &batching) {
    GT lhs, rhs1, rhs2, rhs3;
    lhs = 1, rhs3 = 1;
    pairing(rhs1, batching.witness1, crs.PK1.second - crs.g2 * i);
    pairing(rhs2, batching.witness2, crs.PK2.second - crs.g2 * j);
    for (int t = 0; t < commitments.size(); t++) {
        GT tmp;
        pairing(tmp, commitments[t] * batching.r_batching[t], crs.g2);
        lhs *= tmp;
